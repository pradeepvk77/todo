 ...
 * 
 * Safely adds new columns and tables, backfills safe defaults for existing tasks,
 * migrates historical task_completions into task_occurrences, and logs a comprehensive audit report.
 */

const { sql, initDb } = require("../src/lib/db");
const { detectNoActionOccurrences } = require("../src/lib/no-action-detector");
...
async function runMigration() {
  ...
  const [preTodos] = await sql`SELECT COUNT(*) FROM todos`;
  const [preCompletions] = await sql`SELECT COUNT(*) FROM task_completions`;
  ...
  const [postTodos] = await sql`SELECT COUNT(*) FROM todos`;
  const [postCompletions] = await sql`SELECT COUNT(*) FROM task_completions`;
  const [postOccurrences] = await sql`SELECT COUNT(*) FROM task_occurrences`;
  const [postActivities] = await sql`SELECT COUNT(*) FROM task_activities`;
