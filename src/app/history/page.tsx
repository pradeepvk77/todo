import { getFriendNicknamePreference, getTaskHistory } from "@/app/actions";
...
export default async function HistoryPage({ searchParams }: { searchParams: Promise<{ date?: string; user?: string }> }) {
  const session = await getSession();
  if (!session) redirect("/login");
  const { date: requestedDate, user } = await searchParams;
  ...
  const [{ tasks, completed }, friendNickname] = await Promise.all([getTaskHistory(date, otherUser), getFriendNicknamePreference()]);
